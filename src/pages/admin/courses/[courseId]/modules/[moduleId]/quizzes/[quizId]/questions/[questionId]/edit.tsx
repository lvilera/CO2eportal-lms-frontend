// pages/admin/courses/[courseId]/modules/[moduleId]/quizzes/[quizId]/questions/[questionId]/edit.tsx
...
export default function EditQuestionPage() {
  ...
  useEffect(() => {
    ...
    const fetchQuestion = async () => {
      try {
        setLoading(true);
        const res = await apiRequest.get(`/quizzes/questions/${questionId}`);
        const data = res.data;
        ...
        let options: QuestionOption[] = Array.isArray(data.options)
          ? data.options
          : [];
        ...
        setForm({
          ...
          timeLimitSeconds:
            typeof data.timeLimitSeconds === "number"
              ...
              : 0,
          difficulty:
            data.difficulty === "easy" || data.difficulty === "hard"
              ? data.difficulty
              : "medium",
        });
  ...
  const submit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setSaving(true);
    try {
      await apiRequest.put(`/quizzes/questions/${questionId}`, {
        quizId,
        type: form.type,
        text: form.text,
      ...
      // Redirect after update – usually back to the questions list
